 ...
 */
public class RankPCSUserAdapter extends BaseAdapter {
    private JSONArray data=null;
    private Context context;
    private int days=1;
    private int flags;

    public RankPCSUserAdapter(Context context, JSONArray data, int days,int flags) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String phone=null;
        if (flags==1){
            phone = ((JSONObject)data.get(position)).getString("SHYH");
        }else {
            phone = ((JSONObject)data.get(position)).getString("CJYH");
        }
