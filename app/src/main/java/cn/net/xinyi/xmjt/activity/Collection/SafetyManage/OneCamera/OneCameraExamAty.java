 ...
 * @author
 */
public class OneCameraExamAty extends BaseActivity2 {

	private ImageView leftIv;
	private TextView titleTv;
	private TextView right;
	VoteSubmitViewPager viewPager;
	OneCameraExamSubmitAdapter pagerAdapter;
	...
	private void loadData(){
		...
		ApiResource.SaftyQuestionRandom(json, new AsyncHttpResponseHandler() {
			@Override
			public void onSuccess(int i, Header[] headers, byte[] bytes) {
				...
				if (result.length()>4){
					...
					pagerAdapter = new OneCameraExamSubmitAdapter(OneCameraExamAty.this, viewItems, dataItems,imgServerUrl);
			...
			public void onFailure(int i, Header[] headers, byte[] bytes, Throwable throwable) {
				stopLoading();
				BaseUtil.showDialog("获取数据失败", OneCameraExamAty.this);
			}
		});
...
//		}
		...
		DialogHelper.showAlertDialog( OneCameraExamAty.this,
	...
	private void upateData() {
		...
		ApiResource.SaftyExamineAdd(JSON.toJSONString(approvalModle),new AsyncHttpResponseHandler() {
			@Override
			public void onSuccess(int i, Header[] headers, byte[] bytes) {
				String result = new String(bytes);
				if (result!=null&&result.length()>4){
					UI.toast(OneCameraExamAty.this,"上传成功！");
					setResult(RESULT_OK);
					OneCameraExamAty.this.finish();
				}else {
					onFailure(i,headers,bytes,null);
				}
				stopLoading();
			...
			public void onFailure(int i, Header[] headers, byte[] bytes, Throwable throwable) {
				stopLoading();
				UI.toast(OneCameraExamAty.this,"上传失败！");
			}
		});
	}
