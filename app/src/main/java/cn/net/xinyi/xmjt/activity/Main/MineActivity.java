...
import cn.net.xinyi.xmjt.v527.presentation.mine.ModifyUseInfoActivity;
 ...
 */
public class MineActivity extends BaseActivity2 implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.config_actitiy);
        initView();
        initData();
        initListener();
    }
   ...
   /* @Nullable
    ...
    private void initView() {
        clearcatch = (TextView) findViewById(R.id.clear_catch);//清除缓存
        contactsdb = (TextView) findViewById(R.id.contactsdb);//更新通讯录
        blackcardb = (TextView) findViewById(R.id.blackcardb);//更新黑名单
        loginout = (LinearLayout) findViewById(R.id.login_out);//注销
        about = (LinearLayout) findViewById(R.id.about);//关于
        update = (LinearLayout) findViewById(R.id.update);//版本更新
        tv_update = (TextView) findViewById(R.id.tv_update);//当前版本
        offlinemap = (LinearLayout) findViewById(R.id.offlinemap);//离线地图
        feedback = (LinearLayout) findViewById(R.id.feedback);//意见反馈
        modifypassword = (LinearLayout) findViewById(R.id.modify_password);//修改密码
        personInfo = (LinearLayout) findViewById(R.id.personInfo);//人员信息
        img_head = (ImageView) findViewById(R.id.img_head);//人员信息
        tv_sf = (TextView) findViewById(R.id.tv_sf);
        tv_dw = (TextView) findViewById(R.id.tv_dw);
        tv_yjfk = (TextView) findViewById(R.id.tv_yjfk);//意见反馈
    ...
    private void initData() {
        ...
        ImageLoaderUtils.showImage(img_head, IMAGE_HOST + "/user/" + userInfo.getTxzp());
        String currentAppVersion = "1.0";
        try {
            currentAppVersion = MineActivity.this.getPackageManager().getPackageInfo(MineActivity.this.getPackageName(), 0).versionName;
        ...
        if (networkType == 0) {
            BaseUtil.showDialog(getResources().getString(R.string.msg_check_version_no_network), this);
        }
        ...
        if (AppContext.instance.getProperty(AppConfig.BLACKPLATE_UPDATE_TIME) != null) {
            ...
            int count = new BlackPlateDBHelper(this).getBlackListCount();
    ...
    public void onClick(View v) {
        int networkType = ((AppContext) this.getApplication()).getNetworkType();
        if (networkType == 0) {
            BaseUtil.showDialog(getResources().getString(R.string.msg_check_version_no_network), this);
        } else {
            switch (v.getId()) {
                ...
                case R.id.personInfo:
                    if (BaseDataUtils.isCompanyOrOther() == 1) {
                        ...
                        showActivity(ModifyUseInfoActivity.class);
                ...
                case R.id.offlinemap:
                    if (networkType != 1) {
                        BaseUtil.showDialog(getResources().getString(R.string.msg_check_version_wifi_only), this);
                        break;
                    }
                ...
                case R.id.feedback:
                    if (!((AppContext) this.getApplication()).isNetworkConnected()) {
                        BaseUtil.showDialog(getResources().getString(R.string.msg_offline_feedback), this);
                    } else {
    ...
    private void login_out() {
        DialogHelper.showAlertDialog("将要退出当前账号，确定注销账号吗？", this, new DialogHelper.OnOptionClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which, Object o) {
                ...
                MineActivity.this.stopService(new Intent(MineActivity.this, BaiduTraceService.class));
    ...
    private Handler mHandle = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0:
                    mProgressDialog = new ProgressDialog(MineActivity.this);
                ...
                case 1:
                    ...
                    BaseUtil.showDialog("同步更新通讯录成功", MineActivity.this);

                    break;
                case 2:
                    if (mProgressDialog != null && mProgressDialog.isShowing())
                        mProgressDialog.cancel();
                    BaseUtil.showDialog("同步更新通讯录失败", MineActivity.this);
                    break;
                case 3:
                    mProgressDialog = new ProgressDialog(MineActivity.this);
                ...
                case 4:
                    if (mProgressDialog != null && mProgressDialog.isShowing())
                        mProgressDialog.cancel();
                    BaseUtil.showDialog("本地电话本更新成功", MineActivity.this);
                    break;
                case 5:
                    mProgressDialog = new ProgressDialog(MineActivity.this);
                ...
                case 6:
                    ...
                    int count = msg.arg1;
                    if (count == 0) {
                        BaseUtil.showDialog("车牌比对库已经是最新版本，无需更新。", MineActivity.this);
                    } else {
                        BaseUtil.showDialog(
                ...
                case 7:
                    if (mProgressDialog != null && mProgressDialog.isShowing())
                        mProgressDialog.cancel();
                    BaseUtil.showDialog("同步更新车牌比对库失败", MineActivity.this);
                    break;
                ...
                case 13:
                    ...
                    mProgressDialog.setCancelable(false);
                    mProgressDialog.show();
                    cleanApplicationData(MineActivity.this);
                    break;
    ...
    private void getPlateBlackDb() {
        ...
        if (!dbFile.exists()) {
            BlackPlateDBHelper.initDatabase(this);
        }
        ...
        ApiResource.getPlateBlack(lastUpateTime, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, Header[] headers, byte[] bytes) {
                ...
                if (jsonArray.size() > 0) {
                    BlackPlateDBHelper dbHelper = new BlackPlateDBHelper(MineActivity.this);
...
//                        updateLocalContacts();
                        //更新本地缓存的通讯录；
                        ContactsHelper.updateLocalContacts(MineActivity.this, allContacts);
    ...
    public void cleanApplicationData(Context context) {
        ...
        BaseUtil.showDialog("缓存清理成功", this);
    }
