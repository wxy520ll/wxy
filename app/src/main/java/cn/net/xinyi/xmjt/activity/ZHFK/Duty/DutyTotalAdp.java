 ...
 */
public class DutyTotalAdp extends BaseAdapter {
...
//    }
    ...
    public DutyTotalAdp(Context context, JSONArray data) {
...
//        XLRS:巡逻人数
//        ZGSL:值岗人数量
        String CJRS =((JSONObject)data.get(position)).getString("CJRS");
        String PCSMC =((JSONObject)data.get(position)).getString("PCSMC");
        String XLDS =((JSONObject)data.get(position)).getString("XLDS");
        String XLRS =((JSONObject)data.get(position)).getString("XLRS");
        String ZGSJ =((JSONObject)data.get(position)).getString("ZGSL");
        viewHolde.tv_cjrs.setText(CJRS == null ? "0" : CJRS );
        viewHolde.tv_pcs.setText(PCSMC);
        viewHolde.tv_xdzs.setText(ZGSJ== null ? "0" : ZGSJ);
        viewHolde.tv_sgs.setText(XLDS== null ? "0"  : XLDS);
        viewHolde.tv_kyjl.setText(XLRS== null ? "0" : XLRS);
        return convertView;
    }
