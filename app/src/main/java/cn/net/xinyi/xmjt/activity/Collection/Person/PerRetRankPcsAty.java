...
//采集派出所排名
public class PerRetRankPcsAty extends BaseActivity implements View.OnClickListener {
    public static final String TAG = PerRetRankPcsAty.class.getName();
/***选择开始时间*/
    ...
    private void getRank(){
        ...
        ApiResource.PerPCSRank(json, new AsyncHttpResponseHandler() {
            @Override
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 2:
                    if (mProgressDialog != null && mProgressDialog.isShowing())
                        mProgressDialog.cancel();
                    BaseUtil.showDialog("获取数据失败", PerRetRankPcsAty.this);
                    break;
                default:
                    break;
            }
            ...
            case  R.id.tv_choose_start_date:
                String start_date=tv_choose_start_date.getText().toString().trim();
                 new DatePickerDialog(
                        PerRetRankPcsAty.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
                    public void onDateSet(DatePicker view,
                        ...
                        try {
                            ...
                            if (choose_start_date<init_date){
                                Toast.makeText(PerRetRankPcsAty.this, "开始日期不能小于2015-10-26",Toast.LENGTH_SHORT).show();
                                //结束时间和开始时间进行比较
                            } else if (choose_end_date<choose_start_date){
                                Toast.makeText(PerRetRankPcsAty.this, "开始日期不能大于结束日期",Toast.LENGTH_SHORT).show();
                            }else{
