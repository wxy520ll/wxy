...
//            });
    ...
    protected void changeCheckBtnWithPosition(int position){
        ...
        if(!selectedphotos.isEmpty()){
            if (selectedphotos.contains(photos.get(position)) || isContainPhotos(photos.get(position))){
    ...
    private boolean isContainPhotos(PhotoModel model){
        boolean isContain = false;
        for (PhotoModel temp: selectedphotos){
            String  tempcopy  = temp.getOriginalPath();
            if (temp.getOriginalPath().startsWith("file://")){
                tempcopy = temp.getOriginalPath().replace("file://","");
            }

            if(tempcopy.equals(model.getOriginalPath())){
                isContain = true;
                break;
            }
        }
